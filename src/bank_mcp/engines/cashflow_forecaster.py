...
from bank_mcp.store import subscription_creep as sc      # field extraction / grouping / cadence
from bank_mcp.engines import recurring as rec              # recurring streams (inflow + outflow)
from bank_mcp.store import obligation_registry as oblreg # confirmed forward-plan obligations
from bank_mcp.report import delivery                      # canonical narrate + send_email + money
...
def render(summary):
    h = summary["headline"]
    L = []
    L.append("# bank.mcp — CASH-FLOW / OVERDRAFT FORECAST")
...
def main():
    ...
    if a.email is not None:
        ...
        subj = (f"bank.mcp — Cash-Flow Forecast {today} "
