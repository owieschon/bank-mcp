#!/usr/bin/env python3
"""
fee_fraud_scan.py — fee + fraud/error hunter for the bank.mcp suite.
...
def scan(txns, days=30, overrides=None, reconciliation=None):
    ...
    try:
        from bank_mcp.store import obligation_registry as _oblreg
...
def render(s):
    ...
    d = s["detail"]
    L = []
    L.append("# bank.mcp — FEE + FRAUD SCAN")
...
def main():
    ...
    if a.email is not None:
        ...
        subj = f"bank.mcp — Fee + Fraud Scan {s['window']['end']}"
